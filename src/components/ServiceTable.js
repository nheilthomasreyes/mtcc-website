...
export function ServiceTable({ clients, tests = [],  onEdit, onDelete, onComplete }) {
  ...
  const filteredClients = useMemo(() => {
    return clients.filter(client => {
      ...
      // Check if search contains multiple words (e.g., "josh bsu")
      const searchWords = searchTerm.split(/\s+/).filter(word => word.length > 0);
      
      let matchesSearch = false;
      
      if (searchWords.length >= 2) {
        // Multi-word search: Check if name contains some words AND company contains other words
        // OR if the entire combined string matches
        const combinedText = `${clientName} ${clientCompany}`;
        
        // Check if all search words appear in the combined name + company
        const allWordsMatch = searchWords.every(word => 
          clientName.includes(word) || clientCompany.includes(word)
        );
        
        // Also check if the full search term appears in combined text
        const fullTermMatch = combinedText.includes(searchTerm);
        
        matchesSearch = allWordsMatch || fullTermMatch;
      } else {
        // Single word search: match in either name OR company
  ...
  const exportToExcel = async () => {
    try {
      ...
      // Filter data by the selected year, similar to TallyModal logic
