...
export function ServiceTable({ clients, onEdit, onDelete, onComplete }) {
  ...
  const getServiceRequestName = (client, allClients) => {
    ...
    try {
      ...
      // Set up Columns based on Service Table data
      ws.columns = [
        ...
        { header: 'Specimen No.', key: 'specimenNo', width: 15 },
      ...
      // Filter data by the selected year, similar to TallyModal logic [cite: 13]
      ...
      dataToExport.forEach(client => {
        const row = ws.addRow({
          ...
          srf: getServiceRequestName(client, clients), // Autogenerated SRF [cite: 23, 27]
        ...
        // Styling the Checkbox cells (Centering and Font size)
        ...
        [orCell, roaCell].forEach(cell => {
          ...
          // Optional: Green color for checked, Red for unchecked
          if (cell.value === '☑') {
            cell.font = { size: 14, bold: true, color: { argb: 'FF008000' } };
          }
        });
      });

      // Styling the Header (using theme colors from TallyModal) [cite: 50, 66]
    ...
    <div className="rounded-2xl bg-black/40 backdrop-blur-xl border border-white/10 overflow-hidden flex flex-col h-[calc(100vh-230px)]">
      ...
      <div className="flex-none p-6 border-b border-white/10 space-y-4 bg-slate-900/50 relative z-50">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <button onClick={() => setStatusFilter('All')} className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${statusFilter === 'All' ? 'bg-gradient-to-r from-cyan-500 to-blue-600 text-white shadow-lg shadow-cyan-500/50 border border-cyan-500' : 'bg-white/5 text-gray-300 border border-white/10 hover:bg-white/10'}`}>
            All ({clients.length})
          </button>
          <button onClick={() => setStatusFilter('Ongoing')} className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${statusFilter === 'Ongoing' ? 'bg-gradient-to-r from-blue-500 to-cyan-500 text-white shadow-lg shadow-blue-500/50 border border-blue-500' : 'bg-white/5 text-gray-300 border border-white/10 hover:bg-white/10'}`}>
            Ongoing ({clients.filter(c => c.status === 'Ongoing').length})
          </button>
          <button onClick={() => setStatusFilter('Pending')} className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${statusFilter === 'Pending' ? 'bg-gradient-to-r from-amber-500 to-orange-500 text-white shadow-lg shadow-amber-500/50 border border-amber-500' : 'bg-white/5 text-gray-300 border border-white/10 hover:bg-white/10'}`}>
            Pending ({clients.filter(c => c.status === 'Pending').length})
          </button>
          <button onClick={() => setStatusFilter('Completed')} className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${statusFilter === 'Completed' ? 'bg-gradient-to-r from-green-500 to-emerald-500 text-white shadow-lg shadow-green-500/50 border border-green-500' : 'bg-white/5 text-gray-300 border border-white/10 hover:bg-white/10'}`}>
            Completed ({clients.filter(c => c.status === 'Completed').length})
          </button>
          <button onClick={() => setStatusFilter('Cancelled')} className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${statusFilter === 'Cancelled' ? 'bg-gradient-to-r from-red-500 to-rose-500 text-white shadow-lg shadow-red-500/50 border border-red-500' : 'bg-white/5 text-gray-300 border border-white/10 hover:bg-white/10'}`}>
            ...
            {paginatedClients.map((client, index) => {
              ...
              return (
                <tr key={client.id ?? index} className={`hover:bg-white/5 transition-colors ${client.doNotDelete ? 'bg-red-500/5' : ''}`}>
                  ...
                  <td className="px-5 py-5">
                    {/* Changed to a grid with 3 columns to ensure exactly 3 items per row */}
